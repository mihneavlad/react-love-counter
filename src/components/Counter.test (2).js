...
describe('Counter', () => {
  ...
  xit('renders one .increment button', () => {
    const buttons = component.find('button.increment');
    expect(buttons.length).toBe(1);
  });

  xit('renders one .decrment button', () => {
  ...
  // up by all of you
  xit('doesn\'t have it\'s own state*', () => {
  ...
  describe('An .increment button', () => {
    ...
    // Not a good test really, but just to make sure the state was really lifted
    // up by all of you
    xit('doesn\'t change state on click*', () => {
      button.simulate('click');
      expect(component.state()).toBeNull();
    });
  });
  ...
  describe('A .decrement button', () => {
    ...
    xit('calls the onDecrement function from props when clicked', () => {
      button.simulate('click');
